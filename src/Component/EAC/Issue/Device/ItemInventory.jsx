...
const ItemInventory = ({
  ...
  const handleConfirmSubmitRequest = async () => {
    ...
    const fileUidArray = inventoryTransaction.fileUploaded.map(
      (item) => `/files/${item.uid}`
    );
    ...
    console.log("paramsDraft", JSON.stringify(paramsDraft, null, 2));
    // showSwal();
    showLoading();
  ...
  useEffect(() => {
    if (inventoryTransaction?.inventorySettlementDetail) sumTotalProduction();
  }, [inventoryTransaction]);
